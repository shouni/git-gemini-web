...
| **アーキテクチャ** | **依存性注入 (DI)** / **アダプタパターン** | 高い保守性とテスト容易性を実現するためのサーバー設計基盤。**(`builder/server.go` で全ての依存関係の接続を担います)** |
...
│   │   ├── response.go
│   │   └── review.go
│   ├── pipeline/                # 実行フロー制御（全体の指揮者）
│   │   └── pipeline.go
│   ├── runner/                  # ビジネスロジック本体
│   │   ├── review_runner.go     # レビュー実行
│   │   ├── publish_runner.go    # 結果のパブリッシュ
│   │   ├── report_builder.go    # レポート組み立て
│   │   ├── error_report.md
│   │   └── skip_report.md
│   └── server/                  # Web UI ハンドラー
│       └── handler.go           # フォーム表示・送信受付の実体
...
1.  **フォーム受付:**
    ユーザーがフォームを送信すると、`package server` のhandlerがリクエストを処理します（認証済みユーザーのみ）。
2.  **署名付き URL 生成:**
